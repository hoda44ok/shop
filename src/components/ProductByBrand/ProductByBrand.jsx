...
const ProductByBrand = () => {
  ...
  async function getAllBrandProducts() {
    const response = await axios.get(
      `https://beige-alligator-527710.hostingersite.com/public/api/products/brand/${slug}`
    );
  ...
  const { data, isLoading } = useQuery(["brandProducts", slug], getAllBrandProducts);
  ...
  return (
    ...
    //     <span className="block md:inline">Products by Brand:</span>
    ...
    //         >
    //           <div className="w-contain h-fit sm:h-fit overflow-hidden rounded-t-lg bg-gray-100 flex items-center justify-center">
    //             <img
    //               src={product.image}
    //               alt={product.name}
    ...
    //     >
    //       <div
    //         className="bg-white dark:bg-gray-900 p-6 rounded-lg max-w-md w-full mx-auto relative max-h-[90vh] overflow-y-auto"
...
<div className="min-h-screen bg-white dark:bg-gray-900 px-4 sm:px-6 py-10 max-w-7xl mx-auto lg:mb-12 mb-28">
  <button
    onClick={() => navigate(-1)}
    className="mb-10 flex items-center text-[#0A1172] dark:text-white hover:text-blue-600 transition-colors"
  >
    <i className="fas fa-arrow-left mr-2"></i> Back
  </button>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
      {data.map((product) => (
        <div
          ...
          className="bg-white dark:bg-gray-800 rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300"
        >
          <div className="w-full h-48 bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden rounded-t-lg">
            <img
              src={product.image}
              alt={product.name}
